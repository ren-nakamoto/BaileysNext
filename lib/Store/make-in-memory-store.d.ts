...
declare const _default: (config: BaileysInMemoryStoreConfig) => {
    ...
    /**
     * Search messages in the local store by keyword (offline, no network).
     * Returns newest-first matching WAMessage objects.
     */
    searchMessages: (query: string, jid?: string, count?: number) => proto.IWebMessageInfo[];
    /**
     * Search contacts in the local store by name or JID fragment.
     */
    searchContacts: (query: string, count?: number) => import('../Types').Contact[];
    /**
     * Search chats in the local store by name or JID fragment.
     */
    searchChats: (query: string, count?: number) => import('../Types').Chat[];
};
