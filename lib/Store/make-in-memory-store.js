...
exports.default = (config) => {
    ...
    return {
        ...
        /**
         * Search messages in the local store by keyword.
         * Fast, offline — no network request required.
         * @param query  keyword to match (case-insensitive)
         * @param jid    optional — limit to one chat
         * @param count  max results (default 50)
         * @returns array of matching WAMessage objects, newest first
         */
        searchMessages: (query, jid, count = 50) => {
            ...
            const chatKeys = jid ? [jid] : Object.keys(messages);
            for (const chatJid of chatKeys) {
                const store = messages[chatJid];
                if (!store) continue;
                for (const msg of [...store.array].reverse()) {
        ...
        /**
         * Search contacts in the local store by name or JID fragment.
         * @param query  name or JID fragment (case-insensitive)
         * @param count  max results (default 30)
         * @returns array of matching Contact objects
         */
        searchContacts: (query, count = 30) => {
            const q = (query || '').toLowerCase().trim();
            if (!q) return [];
            const results = [];
            for (const [jid, contact] of Object.entries(contacts)) {
                ...
                if (name.includes(q) || verifiedName.includes(q) || phone.includes(q)) {
        ...
        /**
         * Search chats in the local store by name or JID.
         * @param query  chat name or JID fragment (case-insensitive)
         * @param count  max results (default 20)
         * @returns array of matching Chat objects
         */
        searchChats: (query, count = 20) => {
            const q = (query || '').toLowerCase().trim();
            if (!q) return [];
            const results = [];
