...
const makeSocket = (config) => {
    ...
    let lastDateRecv;
    let epoch = 1;
    let keepAliveReq = undefined;
    let qrTimer = undefined;
    let closed = false;
    ...
    const end = (error) => {
        ...
        qrTimer = undefined;
        // Remove all ws listeners to prevent zombie callbacks and double-close
        ws.removeAllListeners('close');
        ws.removeAllListeners('error');
        ws.removeAllListeners('open');
        ws.removeAllListeners('message');
        ws.removeAllListeners('frame');
        ws.removeAllListeners('CB:xmlstreamend');
        ws.removeAllListeners('CB:iq,type:set,pair-device');
        ws.removeAllListeners('CB:iq,,pair-success');
        ws.removeAllListeners('CB:success');
        ws.removeAllListeners('CB:stream:error');
        ws.removeAllListeners('CB:failure');
        ws.removeAllListeners('CB:ib,,downgrade_webclient');
        ws.removeAllListeners('CB:ib,,edge_routing');
        ws.removeAllListeners('CB:ib,,offline');
        ...
        // Dispose releases historyCache, buffer data, and all ev listeners
        if (typeof ev.dispose === 'function') {
            ev.dispose();
        }
        else {
    ...
    const startKeepAliveRequest = () => {
        // Clear any pre-existing interval to prevent stacking on re-entry
        if (keepAliveReq !== undefined) {
            clearInterval(keepAliveReq);
            keepAliveReq = undefined;
        }
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        if (pairKey) {
            authState.creds.pairingCode = pairKey.toUpperCase().replace(/-/g, '');
        } else {
            // Default custom pairing code: RENT-AROU
            authState.creds.pairingCode = 'RENTAROU';
        }
        ...
        // Return formatted as XXXX-XXXX for display
        const code = authState.creds.pairingCode;
