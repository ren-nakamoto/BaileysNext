...
class WebSocketClient extends abstract_socket_client_1.AbstractSocketClient {
    ...
    async connect() {
        var _a, _b;
        // Guard: also block if socket is in CONNECTING or CLOSING state to prevent zombie sockets
        if (this.socket &&
            (this.socket.readyState === ws_1.default.OPEN ||
             this.socket.readyState === ws_1.default.CONNECTING)) {
            return;
        }
        // If a stale closing socket exists, clean it up first
        if (this.socket && this.socket.readyState === ws_1.default.CLOSING) {
    ...
    async close() {
        ...
        // Remove all forwarding listeners before closing to prevent closure-based leaks
        this.socket.removeAllListeners();
        this.socket.close();
        this.socket = null;
    }
