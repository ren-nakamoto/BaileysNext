...
export declare const makeNewsletterSocket: (config: SocketConfig) => {
    ...
    /** Search newsletters/channels by keyword with pagination + filters (May 2026 upgrade). */
    newsletterSearch: (
        searchQuery: string,
        count?: number,
        after?: string | number,
        filters?: { type?: 'all' | 'following'; sort?: 'relevance' | 'subscribers' }
    ) => Promise<{
        results: {
            id: string;
            name: string;
    ...
    newsletterSubscriberCount: (jid: string) => Promise<number | undefined>;
    newsletterFetchAdmins: (jid: string) => Promise<any[]>;
