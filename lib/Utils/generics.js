...
// ── Device Modernization (2026 tuples) ────────────────────────────────────────
exports.Browsers = {
    ubuntu: (browser) => ['Ubuntu', browser, '24.04.2'],
    macOS: (browser) => ['Mac OS', browser, '15.5.0'],      // macOS Sequoia 15.5 (2026)
    baileys: (browser) => ['Baileys', browser, '6.7.21'],
    windows: (browser) => ['Windows', browser, '10.0.26200'], // Windows 11 24H2+ (2026)
    appropriate: (browser) => [PLATFORM_MAP[(0, os_1.platform)()] || 'Ubuntu', browser, (0, os_1.release)()]
};

/**
 * Realistic browser User-Agent strings for 2026 devices
 * Used for fingerprint stability and anti-detection.
 */
exports.DeviceUserAgents = {
    macOS: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 15_5_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36',
    windows: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36',
    android: 'Mozilla/5.0 (Linux; Android 16; Pixel 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.6478.72 Mobile Safari/537.36',
    iphone: 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.5 Mobile/15E148 Safari/604.1',
};

/**
 * Stable session fingerprint — generated once per session, persisted across reconnects.
 * Prevents fingerprint drift that can trigger WA's anomaly detection.
 */
...
const getSessionFingerprint = () => {
    if (!_sessionFingerprint) {
        ...
        _sessionFingerprint = {
            id: rand,
            createdAt: Date.now(),
            platform: PLATFORM_MAP[(0, os_1.platform)()] || 'Ubuntu',
        };
    }
...
/**
 * Format a raw 8-char pair code with configurable separator, prefix, and segment size.
 * @param code     Raw pairing code (8 chars)
 * @param options  Optional formatting config
 */
const formatPairCode = (code, options = {}) => {
    const { separator = '-', prefix = '', segmentSize = 4 } = options;
    const c = code.replace(/-/g, '').toUpperCase();
    if (c.length !== 8) return prefix ? `${prefix}${separator}${c}` : c;
    const seg1 = c.slice(0, segmentSize);
    const seg2 = c.slice(segmentSize);
    const formatted = `${seg1}${separator}${seg2}`;
    return prefix ? `${prefix}${separator}${formatted}` : formatted;
};
exports.formatPairCode = formatPairCode;
...
const generateMessageID = () => 'Laurine-' + (0, crypto_1.randomBytes)(6).toString('hex').toUpperCase();
...
const fetchLatestWaWebVersion = async (options) => {
    try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), ((options === null || options === void 0 ? void 0 : options.timeout) || 10000));
        ...
        clearTimeout(timeout);
        const data = await res.json();
        const version = data.currentVersion.split('.');
        return {
